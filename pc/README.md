...
| `vad.hangover_ms` | 言い切ったあとの終端判定。短いと文の途中で割れる |
| `vad.hangover_ms_short` | 発話がまだ短いときの終端判定。既定 `800` |
| `vad.short_utterance_ms` | 有音がこれ未満なら「まだ続く」とみなす。既定 `1200` |
...
**聞き間違いが多い / 言っていないのに変な文が認識される**
発話が途中で割れている可能性が高いです。断片を渡された whisper は認識を
崩し、末尾の断片では幻聴を出します。実測では 300ms の間でも割れていました。

`--check-mic` の「VAD の切り出し」を見て、1回の発話が2本以上に分かれて
いないか確認してください。分かれているなら `vad.hangover_ms_short` を
1000 程度まで上げます。
